...
function TableCashflowIndirecto(props) {
  ...
  useEffect(() => {
    if (FEOperativas && FEfinanciacion && inversiones) {
      ...
      for (let i = 0; i < 10; i++) {
        resultado.push(
          isNaN(FEOperativas[i] + FEfinanciacion[i] + inversiones[i])
            ? 0
            : FEOperativas[i] + FEfinanciacion[i] + inversiones[i],
        );
      }
    ...
    if (variacionCajaYBco) {
      ...
      for (let i = 0; i < 10; i++) {
        if (i === 0) {
          resultado.push(
            isNaN(
              parseInt(variacionCajaYBco[0]) +
                parseInt(cajaYBancosInicioManual),
            )
              ? 0
              : parseInt(variacionCajaYBco[0]) +
                  parseInt(cajaYBancosInicioManual),
          ...
          resultado.push(
            isNaN(parseInt(variacionCajaYBco[i]) + parseInt(resultado[i - 1]))
              ? 0
                    ...
                    <div className="flex flex-col">
                      ...
                      <FormItem className="mb-0">
                        <Input
                          className="w-[130px]"
                          type="text"
                          value={Number(inputsValues.cajaYBancos)}
                          onChange={(e) =>
